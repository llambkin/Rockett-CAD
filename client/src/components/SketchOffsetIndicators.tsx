...
export function SketchOffsetIndicators() {
  ...
  return <div className="dim-label-layer" ref={layer}>
    {(draft.offsets ?? []).map((offset, i) => <button key={offset.id} data-offset={offset.id}
      className="dim-label offset-label" title={`Edit Offset ${i + 1}`} aria-label={`Edit Offset ${i + 1}, ${offset.distance} mm`}
      disabled={busy} onPointerDown={e => e.stopPropagation()} onDoubleClick={e => e.stopPropagation()}
      onClick={() => {
        const s = useStore.getState();
        s.setSketchTool("offset");
        s.setDialogParams({ editOffsetId: offset.id, sketchOffset: offset.distance });
      }}>↔ Offset {i + 1}: {offset.distance} mm</button>)}
  </div>;
}

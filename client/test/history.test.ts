...
const sketch: SketchFeature = {
  ...
  entities: [{ id: "p", kind: "point", x: 5, y: 0 }],
  constraints: [],
};
const doc = createEmptyDocument("history", "History");
doc.features = [sketch];
  ...
  sketches: [
    {
      ...
      entities: [{ id: "p", kind: "point", x: 10, y: 0 }],
...
it("exits cleanly when undo removes the sketch", async () => {
  vi.mocked(api.replaceDocument).mockResolvedValue({
    ...
    evaluation: { ...evaluation, sketches: [] },
  } as any);
  await useStore.getState().undo();
...
it("ignores a second undo while the first request is pending", async () => {
  ...
  vi.mocked(api.replaceDocument).mockImplementation(
    () =>
      new Promise((resolve) => {
        complete = resolve;
      }),

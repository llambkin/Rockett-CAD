...
import { detectProfiles, solveSketch, type SketchEntity, type SketchConstraint } from "@rockett/shared";
import { createRect, createCenterRect, type Created, type UV } from "../src/sketchTools";

function resize(created: Created, width: number, height: number, extra: SketchEntity[] = [], constraints: SketchConstraint[] = []) {
  const sides = created.entities.filter(e => e.kind === "line" && !e.construction);
  const result = solveSketch({
    entities: [...extra, ...created.entities],
    constraints: [...constraints, ...created.constraints,
...
describe("rectangle starting anchors", () => {
  it.each<UV>([
  ...
  ])("keeps a snapped corner at $snapKind when dimensions change and reopen", anchor => {
    ...
    for (const [w, h] of [[40, 15], [7, 30], [23, 9]]) {
  ...
  it("keeps a centre rectangle centred on a sketch midpoint and follows its reference", () => {
    const rect = createCenterRect({ x: 25, y: 10, snapMidLineId: "ref", snapKind: "midpoint" }, { x: 31, y: 14 });
    ...
    const sides = rect.entities.filter(e => e.kind === "line" && !e.construction);
    for (const endX of [50, 70]) {
      (source[1] as { x: number }).x = endX;
      const solved = resize(rect, 30, 12, source);
      const a = point(solved, sides[0].p1), c = point(solved, sides[1].p2);
  ...
  it("resizes a centre rectangle symmetrically about a face midpoint", () => {
    const rect = createCenterRect({ x: 25, y: 10, snapKind: "midpoint" }, { x: 31, y: 14 });
    const solved = resize(rect, 30, 12);
    const outline = rect.entities.filter(e => e.kind === "line" && !e.construction);
  ...
  it("retains the opposite corner's connection when a rectangle ends on another edge", () => {
    ...
    const rect = createRect({ x: 0, y: -2.2 }, { x: 9.7, y: 0, snapLineId: "edge", snapKind: "curve" });

...
vi.mock("../src/api", () => ({ api: { evaluate: vi.fn(), updateFeature: vi.fn(), replaceDocument: vi.fn() } }));

const sketch: SketchFeature = { id: "sk", type: "sketch", name: "Sketch", suppressed: false,
  plane: { kind: "origin", plane: "XY" }, entities: [{ id: "p", kind: "point", x: 0, y: 0 },
    { id: "circle", kind: "circle", center: "p", radius: 10 }], constraints: [] };
const doc = { ...createEmptyDocument("editing", "Editing"), features: [sketch,
  { id: "ext", type: "extrude", name: "Extrude", suppressed: false, profiles: [], distance: 10, direction: "normal", operation: "newBody" }], timelinePosition: 2 } as any;
const sketchEvaluation = { bodies: [], planes: [], kernelMs: 0, featureStatuses: [], sketches: [{
  featureId: "sk", entities: sketch.entities, profiles: [], dof: 3, solveStatus: "unconstrained",
...
beforeEach(() => {
  vi.clearAllMocks();
  useStore.setState({ document: structuredClone(doc), projectId: doc.id, mode: { name: "idle" },
    evaluation: fullEvaluation, draftSketch: null, busy: false, error: null, undoStack: [], redoStack: [], previewBaseline: null });
  vi.mocked(api.evaluate).mockImplementation(async (_id, position) => position === 1 ? sketchEvaluation : fullEvaluation);
  vi.mocked(api.updateFeature).mockImplementation(async (_id, _fid, patch) => ({
    document: { ...doc, features: [{ ...sketch, ...patch }, doc.features[1]] },
    evaluation: { ...sketchEvaluation, sketches: [{ ...sketchEvaluation.sketches[0], entities: (patch as any).entities }] },
  }));
});
...
it("keeps the draft editable when finishing fails", async () => {
  await useStore.getState().editSketch("sk");
  useStore.setState({ draftSketch: { ...sketch, entities: [...sketch.entities, { id: "extra", kind: "point", x: 4, y: 5 }] } });
...
it("saves offset metadata and distance edits as undoable changes while rolled back", async () => {
  ...
  expect(useStore.getState().document!.features[0]).toMatchObject({ offsets: [{ distance: 4 }] });
  expect(useStore.getState().undoStack.at(-1)!.features[0]).toMatchObject({ offsets: [{ distance: 2 }] });
  expect(api.updateFeature).toHaveBeenLastCalledWith(doc.id, "sk", expect.objectContaining({
...
it("undo during editing retains the temporary evaluation position", async () => {
  ...
  vi.mocked(api.replaceDocument).mockResolvedValue({ document: doc, evaluation: sketchEvaluation });

...
const O = { x: 0, y: 0 };
const field = (key: DimField["key"], text: string, locked = true): DimField => ({
  key,
  label: key,
  unit: "",
  text,
...
describe("typed sizes while drawing", () => {
  it("exposes the right fields per tool", () => {
    expect(dimFieldsFor("line")?.map((f) => f.key)).toEqual(["length", "angle"]);
    expect(dimFieldsFor("rect")?.map((f) => f.key)).toEqual(["width", "height"]);
    expect(dimFieldsFor("centerRect")?.map((f) => f.key)).toEqual(["width", "height"]);
  ...
  it("live values follow the cursor; angles normalise to [0, 360)", () => {
    ...
    expect(liveDimValues("rect", O, { x: -3, y: 7 })).toEqual({ width: 3, height: 7 });
    expect(liveDimValues("centerRect", O, { x: -3, y: 7 })).toEqual({ width: 6, height: 14 });
  ...
  it("returns the cursor untouched (snap ids intact) when nothing is locked", () => {
    ...
    expect(resolveDimCursor("line", O, cursor, dimFieldsFor("line")!)).toBe(cursor);
    expect(resolveDimCursor("rect", O, cursor, dimFieldsFor("rect")!)).toBe(cursor);
  });
  ...
  it("line: locked length keeps the cursor direction; locked angle is exact on the axes", () => {
    ...
    expect(resolveDimCursor("line", O, { x: 3, y: 4 }, l50)).toEqual({ x: 30, y: 40 });

    const up = [field("length", "10"), field("angle", "90")];
    expect(resolveDimCursor("line", { x: 5, y: 5 }, { x: 9, y: 9 }, up)).toEqual({ x: 5, y: 15 });
    const left = [field("length", "10"), field("angle", "180")];
    expect(resolveDimCursor("line", { x: 5, y: 5 }, { x: 9, y: 9 }, left)).toEqual({ x: -5, y: 5 });
    const down = [field("length", "10"), field("angle", "-90")];
    expect(resolveDimCursor("line", { x: 5, y: 5 }, { x: 9, y: 9 }, down)).toEqual({ x: 5, y: -5 });
  ...
  it("pins typed sizes onto the created geometry as dimension constraints", () => {
    ...
    const rc = dimConstraintsFor("rect", rect, [field("width", "10"), field("height", "5")]);
    expect(rc).toEqual([
      expect.objectContaining({ type: "length", line: rectLines[0].id, value: 10 }),
      expect.objectContaining({ type: "length", line: rectLines[1].id, value: 5 }),
    ]);
    // only the typed one is pinned
    expect(dimConstraintsFor("rect", rect, [field("width", "", false), field("height", "5")])).toHaveLength(1);

    const line = createLine(O, { x: 30, y: 40 });
    const lc = dimConstraintsFor("line", line, [field("length", "50"), field("angle", "", false)]);
    expect(lc).toEqual([expect.objectContaining({ type: "length", value: 50 })]);
    ...
    expect(cc).toEqual([expect.objectContaining({ type: "diameter", entity: circleId, value: 8 })]);
  });
});

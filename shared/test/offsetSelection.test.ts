...
import { findOffsetConnector, offsetSketchSelection } from "../src/sketchModify.js";
...
it("joins a box selected out of order without including an unselected branch", () => {
  const source = box();
  const added = offsetSketchSelection(source, [], ["bottom", "top", "left", "right"], 1).entities.slice(source.length);
...
it("waits for connecting curves instead of offsetting part of a disconnected selection", () => {
  expect(() => offsetSketchSelection(box(), [], ["bottom", "top"], 1)).toThrow(/connected chain yet/);
  const source = box();
  const added = offsetSketchSelection(source, [], ["bottom", "top", "right"], 1).entities.slice(source.length);
...
it("does not restore deselected curves when only one manual selection remains", () => {
  const source = box();
  const added = offsetSketchSelection(source, [], ["bottom"], 1, false).entities.slice(source.length);
  expect(added.filter(e => e.kind === "line")).toHaveLength(1);
});

it("rejects a selected branching path and invalid inputs", () => {
  expect(() => offsetSketchSelection(box(), [], ["bottom", "left", "branch"], 1)).toThrow(/branch/);
  expect(() => offsetSketchSelection(box(), [], ["bottom", "a"], 1)).toThrow(/connected lines and arcs/);
  expect(() => offsetSketchSelection(box(), [], ["bottom", "right"], 0)).toThrow(/non-zero/);
});
...
function segmentedOutline(): SketchEntity[] {
  ...
  const segments = [
    [[9.7, 0], [9.7, 16]], [[9.7, 16], [0, 16]], [[-9.7, 16], [0, 16]],
    [[-9.7, 0], [-9.7, 16]], [[9.7, 0], [9.7, -16.275]],
    [[9.7, -18.475], [9.7, -16.275]], [[0, -18.475], [9.7, -18.475]],
    [[0, -18.475], [-9.7, -18.475]], [[-9.7, -18.475], [-9.7, -16.276079261604345]],
    [[-9.7, -16.275], [-9.7, 0]],
  ];
  return segments.flatMap(([a, b], i): SketchEntity[] => [
    { id: `a${i}`, kind: "point", x: a[0], y: a[1] }, { id: `b${i}`, kind: "point", x: b[0], y: b[1] },
...
it("offsets the reported segmented outline across its tiny endpoint gap without editing the source", () => {
  const source = segmentedOutline(), before = JSON.stringify(source);
  ...
  expect(offsetSketchSelection(source, [], ids, -1, false, 0).offsetChain?.closed).toBe(false);
});

it("reports a missing outline segment instead of bridging a real gap", () => {
  const source = segmentedOutline();
  const ids = source.filter(e => e.kind === "line" && e.id !== "edge8").map(e => e.id);
  ...
  expect(detectProfiles(complete.entities.slice(source.length))).toHaveLength(1);
});

it("does not merge the vertices of a valid short segment", () => {
  const source: SketchEntity[] = [
    { id: "a", kind: "point", x: 0, y: 0 }, { id: "b", kind: "point", x: 10, y: 0 },
    { id: "c", kind: "point", x: 10.005, y: 0 }, { id: "d", kind: "point", x: 20, y: 0 },
  ...
  expect(result.entities.slice(source.length).filter(e => e.kind === "line")).toHaveLength(3);
});

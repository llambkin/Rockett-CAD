...
export function projectEdge(
  ...
  if (curve.type === "circle") {
    ...
    if (
      curve.sweep !== undefined &&
      Math.abs(curve.sweep) < Math.PI * 2 - 1e-6
    ) {
      if (!curve.start || !curve.end) throw new Error("Missing arc endpoints.");
      const a = dot > 0 ? curve.start : curve.end,
        b = dot > 0 ? curve.end : curve.start;
      return [
        point("c", curve.center),
        point("a", a),
        point("b", b),
        {
          ...common,
  ...
  throw new Error(
    "Project currently supports straight edges, circles, and circular arcs.",
  );
}

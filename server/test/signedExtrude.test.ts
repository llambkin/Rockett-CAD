...
const square = [P("a", 0, 0), P("b", 20, 0), P("c", 20, 20), P("d", 0, 20), L("l1", "a", "b"), L("l2", "b", "c"), L("l3", "c", "d"), L("l4", "d", "a")];

function build(id: string, distance: number, direction: ExtrudeFeature["direction"], operation: ExtrudeFeature["operation"] = "newBody", startOffset?: number) {
  dropEngine(id);
  const sketch: SketchFeature = { id: "sk", type: "sketch", name: "S", suppressed: false, plane: { kind: "origin", plane: "XY" }, entities: square, constraints: [] };
  const ex: ExtrudeFeature = { id: "ex", type: "extrude", name: "E", suppressed: false, profiles: [], distance, direction, operation, ...(startOffset !== undefined ? { startOffset } : {}) };
  ...
  ex.profiles = [{ sketchId: "sk", profileId: first.sketches[0].profiles[0].id }];
...
describe("signed extrude distance", () => {
  ...
  it("a negative cut removes material from a body behind the sketch plane", () => {
    dropEngine("se4");
    const base: SketchFeature = { id: "sk0", type: "sketch", name: "Base", suppressed: false, plane: { kind: "origin", plane: "XY" }, entities: [P("a", -10, -10), P("b", 40, -10), P("c", 40, 40), P("d", -10, 40), L("l1", "a", "b"), L("l2", "b", "c"), L("l3", "c", "d"), L("l4", "d", "a")], constraints: [] };
    const plate: ExtrudeFeature = { id: "plate", type: "extrude", name: "Plate", suppressed: false, profiles: [], distance: 10, direction: "reverse", operation: "newBody" };
    const sketch: SketchFeature = { id: "sk", type: "sketch", name: "S", suppressed: false, plane: { kind: "origin", plane: "XY" }, entities: square, constraints: [] };
    const cut: ExtrudeFeature = { id: "cut", type: "extrude", name: "Cut", suppressed: false, profiles: [], distance: -4, direction: "normal", operation: "cut" };
    ...
    plate.profiles = [{ sketchId: "sk0", profileId: r.sketches[0].profiles[0].id }];
    r = engine.evaluate(doc, 3);
    cut.profiles = [{ sketchId: "sk", profileId: r.sketches.find((s) => s.featureId === "sk")!.profiles[0].id }];
  ...
  it("validation accepts negative distances but still rejects zero", () => {
    const f = (distance: number): ExtrudeFeature => ({ id: "x", type: "extrude", name: "Extrude", suppressed: false, profiles: [{ sketchId: "s", profileId: "p" }], distance, direction: "normal", operation: "cut" });

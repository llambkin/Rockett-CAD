...
import type { ChamferFeature, EdgeRef, ExtrudeFeature, SketchEntity, SketchFeature } from "@rockett/shared";
...
function starPlate(id: string) {
  dropEngine(id);
  const sketch: SketchFeature = { id: "sk1", type: "sketch", name: "S", suppressed: false, plane: { kind: "origin", plane: "XY" }, entities: star, constraints: [] };
  const ex: ExtrudeFeature = { id: "ex1", type: "extrude", name: "E", suppressed: false, profiles: [], distance: 7, direction: "normal", operation: "join" };
  ...
  ex.profiles = first.sketches[0].profiles.map((p) => ({ sketchId: "sk1", profileId: p.id }));
  const r = engine.evaluate(doc);
  const body = r.bodies[0];
  const loop = (cap: string): EdgeRef[] =>
    body.edges.filter((e) => e.name.includes(cap)).map((e) => ({ kind: "edge", bodyId: body.bodyId, edgeName: e.name }));
  return { engine, doc, top: loop("cap:end"), bot: loop("cap:start"), prismVolume: volumeOf(engine.stateAt(doc).bodies.get(body.bodyId)!.shape) };
}
const chamfer = (id: string, edges: EdgeRef[], distance: number): ChamferFeature => ({ id, type: "chamfer", name: id, suppressed: false, edges, distance });
...
describe("chamfer that consumes a face (3.5 + 3.5 on a 7 mm plate)", () => {
  it("both loops in one feature meet at a knife edge: 18 faces, no walls left", () => {
    ...
    const vol = volumeOf(engine.stateAt(doc).bodies.get(r.bodies[0].bodyId)!.shape);
  ...
  it("top then bottom as two features also succeeds at exactly half the height", () => {
    ...
    const r = engine.evaluate(doc);
    expect(r.featureStatuses.map((s) => s.status)).toEqual(["ok", "ok", "ok", "ok"]);
    expect(r.bodies[0].faces).toHaveLength(18);
    // chamfer faces keep the per-edge naming scheme of the normal path
    expect(r.bodies[0].faces.filter((f) => f.name.startsWith("f:c2:fe:"))).toHaveLength(8);
  });

  it("a star with a central hole (radials construction) chamfers to a knife edge too", () => {
    dropEngine("che5");
    const entities: SketchEntity[] = [
      ...star.map((e) => (e.kind === "line" && e.id.startsWith("rad") ? { ...e, construction: true } : e)),
      P("cc", 0, 0),
      { id: "ci", kind: "circle", center: "cc", radius: 12.5 },
    ];
    const sketch: SketchFeature = { id: "sk1", type: "sketch", name: "S", suppressed: false, plane: { kind: "origin", plane: "XY" }, entities, constraints: [] };
    const ex: ExtrudeFeature = { id: "ex1", type: "extrude", name: "E", suppressed: false, profiles: [], distance: 7, direction: "normal", operation: "join" };
    ...
    const disc = regions.find((p) => p.outer.length === 1 && p.outer[0].entityId === "ci")!;
    ex.profiles = regions.filter((p) => p !== disc).map((p) => ({ sketchId: "sk1", profileId: p.id }));
    let r = engine.evaluate(doc);
    const body = r.bodies[0];
    // 8 walls + 2 caps + the hole wall
    expect(body.faces).toHaveLength(11);
    const loop = (cap: string): EdgeRef[] =>
      body.edges.filter((e) => e.name.includes(cap) && !e.name.includes(":s:ci")).map((e) => ({ kind: "edge", bodyId: body.bodyId, edgeName: e.name }));
    doc.features.push(chamfer("c1", loop("cap:end"), 3.5), chamfer("c2", loop("cap:start"), 3.5));
    doc.timelinePosition = 4;
  ...
  it("an ordinary chamfer still goes through the kernel's chamfer algorithm", () => {
    ...
    expect(r.bodies[0].faces).toHaveLength(18);
    expect(r.bodies[0].faces.filter((f) => f.name.startsWith("f:c1:fe:"))).toHaveLength(8);
  });

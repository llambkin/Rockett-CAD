...
import type { ExtrudeFeature, SketchEntity, SketchFeature } from "@rockett/shared";
import { createEmptyDocument } from "@rockett/shared";
import { getKernel, initKernel, solids, volumeOf } from "../src/geometry/kernel.js";
...
function extrudeAllRegions(
  ...
  extrude.profiles = first.sketches[0].profiles.map((p) => ({ sketchId: "sk1", profileId: p.id }));
  const result = engine.evaluate(doc);
  return { result, regions: first.sketches[0].profiles.length, engine, doc };
}

const twoRects = () => [
  P("a", 0, 0), P("b", 10, 0), P("c", 20, 0), P("d", 20, 10), P("e", 10, 10), P("f", 0, 10),
  L("l1", "a", "b"), L("l2", "b", "c"), L("l3", "c", "d"), L("l4", "d", "e"), L("l5", "e", "f"), L("l6", "f", "a"),
  L("mid", "b", "e"),
];
...
describe("multi-region extrude: New body keeps regions apart, Join merges them", () => {
  it("removes the shared seam when adjacent regions are extruded in separate features", () => {
    ...
    doc.features.push({ ...first, id: "ext2", name: "Extrude2", profiles: [right], operation: "join" });
  ...
  it("New body makes one body per region, ids b:<feature> and b:<feature>:2", () => {
    const { result, engine, doc } = extrudeAllRegions("uni0", twoRects(), 5, "newBody");
    expect(result.featureStatuses.map((s) => s.status)).toEqual(["ok", "ok"]);
    expect(result.bodies.map((b) => b.bodyId).sort()).toEqual(["b:ext1", "b:ext1:2"]);
    for (const b of result.bodies) expect(b.faces).toHaveLength(6);
    const state = engine.stateAt(doc);
    expect(volumeOf(state.bodies.get("b:ext1")!.shape)).toBeCloseTo(10 * 10 * 5, 2);
    expect(volumeOf(state.bodies.get("b:ext1:2")!.shape)).toBeCloseTo(10 * 10 * 5, 2);
  });

  it("Join of two rectangles sharing an edge gives a plain box: 6 faces, one cap name", () => {
    const { result, regions, engine, doc } = extrudeAllRegions("uni1", twoRects(), 5);
    ...
    expect(volumeOf(engine.stateAt(doc).bodies.get("b:ext1")!.shape)).toBeCloseTo(20 * 10 * 5, 2);
  });
  ...
  it("the ninja star's 12 regions joined extrude to one 10-face solid", () => {
    ...
    const { result, regions, engine, doc } = extrudeAllRegions("uni2", entities, 4);
    ...
    expect(volumeOf(engine.stateAt(doc).bodies.get("b:ext1")!.shape)).toBeCloseTo(7500 * 4, 1);
  });

  it("a circle split by a crossing line extrudes as two half-disc bodies", () => {
    dropEngine("uni4");
    const entities: SketchEntity[] = [
      P("a", 0, 0), P("b", 40, 0), P("c", 40, 20), P("d", 0, 20),
      L("l1", "a", "b"), L("l2", "b", "c"), L("l3", "c", "d"), L("l4", "d", "a"),
      P("cc", 20, 10), { id: "ci", kind: "circle", center: "cc", radius: 5 },
      P("m1", 0, 10), P("m2", 40, 10), L("cut", "m1", "m2"),
    ];
    const sketch: SketchFeature = {
      id: "sk1", type: "sketch", name: "Sketch1", suppressed: false,
      plane: { kind: "origin", plane: "XY" }, entities, constraints: [],
    };
    const extrude: ExtrudeFeature = {
      id: "ext1", type: "extrude", name: "Extrude1", suppressed: false,
    ...
    extrude.profiles = halves.map((p) => ({ sketchId: "sk1", profileId: p.id }));
    const result = engine.evaluate(doc);
    expect(result.featureStatuses.map((s) => s.status)).toEqual(["ok", "ok"]);
    expect(result.bodies).toHaveLength(2);
    const state = engine.stateAt(doc);
    for (const id of ["b:ext1", "b:ext1:2"]) {
      expect(volumeOf(state.bodies.get(id)!.shape)).toBeCloseTo((Math.PI * 25 * 10) / 2, 1);
    }
  });

  it("a single region is untouched by the merge", () => {
    const entities = [
      P("a", 0, 0), P("b", 30, 0), P("c", 30, 20), P("d", 0, 20),
      L("l1", "a", "b"), L("l2", "b", "c"), L("l3", "c", "d"), L("l4", "d", "a"),
    ];
    const { result } = extrudeAllRegions("uni3", entities, 10);
    const names = result.bodies[0].faces.map((f) => f.name).sort();
    expect(names).toEqual(
      ["f:ext1:cap:end", "f:ext1:cap:start", "f:ext1:s:l1", "f:ext1:s:l2", "f:ext1:s:l3", "f:ext1:s:l4"].sort()
    );
  });
});

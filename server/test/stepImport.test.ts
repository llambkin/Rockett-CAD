...
it("imports multiple B-Rep bodies, persists them, and supports downstream fillets", () => {
  ...
  doc.features = [{ id: "step", type: "importStep", name: "Imported", filename: "fixture.step", data: stepFixture(true), suppressed: false }];
  doc.timelinePosition = 1;
  let engine = engineFor(doc.id), result = engine.evaluate(doc);
  expect(result.featureStatuses[0]).toEqual({ featureId: "step", status: "ok" });
  expect(result.bodies).toHaveLength(2);
  expect(result.bodies[0].bbox.max).toEqual([20, 30, 10].map(x => expect.closeTo(x, 4)));
  ...
  dropEngine(doc.id); engine = engineFor(doc.id);
  result = engine.evaluate(JSON.parse(JSON.stringify(doc)));
  expect(result.bodies[0].faces.map(f => f.name)).toEqual(names);
  doc.features.push({ id: "fillet", type: "fillet", name: "Fillet", suppressed: false,
    edges: [{ kind: "edge", bodyId: "b:step", edgeName: result.bodies[0].edges[0].name }], radius: 1 });
  doc.timelinePosition = 2;
  expect(engine.evaluate(doc).featureStatuses.at(-1)!.status).toBe("ok");
  expect(volumeOf(engine.stateAt(doc).bodies.get("b:step")!.shape)).toBeLessThan(6000);
  dropEngine(doc.id);
});
